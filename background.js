...
// --- GitHub update (sidebar open only; skip during monitoring) ---
...
function canReloadNow(targetVersion) {
  return new Promise(function(resolve) {
    chrome.storage.local.get(['lastReloadAt', 'lastReloadForVersion'], function(d) {
      if (d.lastReloadForVersion === targetVersion) { resolve(false); return; }
      if (d.lastReloadAt && Date.now() - d.lastReloadAt < RELOAD_COOLDOWN_MS) { resolve(false); return; }
      resolve(true);
    });
  });
}
...
function markReloadDone(targetVersion) {
  chrome.storage.local.set({ lastReloadAt: Date.now(), lastReloadForVersion: targetVersion });
}
  ...
  return new Promise(function(resolve) {
    try {
      chrome.storage.local.get(['monitoring'], function(d) {
        if (d.monitoring) { resolve(false); return; }
...
function scheduleNext() {
  chrome.storage.local.get(['config', 'monitoring'], (data) => {
    ...
    // Interval is configured in SECONDS. No minimum is imposed — your values are
    // used as-is.
    ...
    let sec;
    if (hi - lo >= 1) {
      // Proper range → random between min and max.
      sec = lo + Math.random() * (hi - lo);
    } else {
      // Single value (min == max) → jitter symmetrically around it (~±15%) so
      // each interval still differs, without raising your value.
      const jitter = Math.max(3, lo * 0.15);
      sec = lo + (Math.random() - 0.5) * 2 * jitter;
      if (sec < 1) sec = lo;
    }
